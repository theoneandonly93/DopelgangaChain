...
export function WalletWrapper({ children }: { children: ReactNode }) {
  ...
  const rawEndpoint = process.env.NEXT_PUBLIC_SOLANA_RPC || '/api/rpc';
  const endpoint = rawEndpoint.startsWith('http')
    ? rawEndpoint
    : (typeof window !== 'undefined' ? `${window.location.origin}${rawEndpoint}` : 'https://api.mainnet-beta.solana.com');
